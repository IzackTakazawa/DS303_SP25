...
-   **(Intercept)**: -43.781
    ...
    -   This is the log-odds of being classified as "versicolor" when petal length is zero
    -   This value is not practically meaningful as petal lengths in the data set are positive.

-   **your predictor**: 9.002

    -   This coefficient represents for each one-unit increase in petal length, the log-odds of a flower being classified as "virginica" significantly increases compared to "versicolor".
    -   Since the coefficient is positive, it suggest that as petal length increases, the log-odds of being "virginica" than "versicolor"
...
```{r}
in_class <- glm(Species_num ~ Petal.Width + Petal.Length, data = iris_subset, family = binomial)
summary(in_class)
```
...
-   **(Intercept)**: -45.272
    ...
    -   This is the log-odds of the petal being classified as "versicolor" when petal length is zero

-   Petal Width: 10.447

    -   This coefficient represents for each one-unit increase in petal width, the log-odds of a flower being classified as "virginica" significantly increases compared to "versicolor" by e\^10.447 - so significantly.

-   Petal Length: 5.755

    -   This coefficient represents for each one-unit increase in petal length, the log-odds of a flower being classified as "virginica" significantly increases compared to "versicolor" by e\^5.755 - so significantly.
