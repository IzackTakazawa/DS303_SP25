...
A lot more data and correlation between the variables the higher the power

```{r}
ggplot(Boston, aes(x = lstat, y = medv)) +
  geom_point() +
  geom_smooth(method = "lm", formula = y ~ x + I(x^5), color = "blue")
...
```{r}
lm.fit15 <- lm(medv ~ poly(lstat, 15), data = Boston)
summary(lm.fit15)

anova(lm.fit5, lm.fit15)
```
...
```{r}
lm.age <- lm(lstat ~ age, data = Boston)
...
```{r}
lm.age <- lm(lstat ~ poly(age, 2), data = Boston)
summary(lm.age)

anova(lm.fit5, lm.age)
```
